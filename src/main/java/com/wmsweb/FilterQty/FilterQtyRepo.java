...
public interface FilterQtyRepo extends CrudRepository<FilterQty, Long> {
    ...
    @Transactional
    public int insertData(String sku, int qty, String date, String line_no, long batch_no,String bay);


    @Modifying
    @Query(value = "update  filter_qty set qty=?2 where  date=?3 and sku=?1 and bay=?6 and line_no=?4 and batch_no=?5", nativeQuery = true)
    @Transactional
    public int updateData(String sku, int qty, String date, String line_no, long batch_no,String bay);
    ...
    public List<FilterQty> getFilterData(String date, String sku, String line_no, long batch_no,String bay);
