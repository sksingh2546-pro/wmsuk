...
public interface ProductionRepository extends CrudRepository<Production, Long> {
    ...
    @Transactional
    int insertProduction(long batch_no, String date, int qty, String sku, String bay_no, String status);
    ...
    @Transactional
    int updateProduction(long batch_no, String date, int qty, String sku, String bay_no, String status);

    @Query("select p from Production p where batch_no=?1 and sku=?2 and bay_no=?3 and status=?4")
    List<Production> getProductionData(long batch_no, String sku, String bay_no, String status);
    ...
    @Query("select p from Production p where sku=?1 and status=?2 order by batch_no asc")
    ...
    List<Production> getSearchProduct(String sku, long batch_no, String bay_no);
